...
def main(token, repo_name, issue_number=None, dir_name=BACKUP_DIR, skip_issues_rss=False):
    user = login(token)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-issues-rss", action="store_true", help="跳过 Issues 来源的 RSS 生成"
    )
    options = parser.parse_args()
    main(options.github_token, options.repo_name, options.issue_number, skip_issues_rss=options.skip_issues_rss)
